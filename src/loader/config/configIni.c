...
int createDefaultIni(const char *filePath)
{
    ...
    fprintf(file, "# WMMT3DX+ uses two 12-digit dongle IDs. Leave blank to use virtual IDs.\n");
    fprintf(file, "DONGLE_ID = \"%s\"\n", defaults.namcoN2.dongleId);
    fprintf(file, "DONGLE_ID_2 = \"%s\"\n", defaults.namcoN2.dongleId2);
    fprintf(file, "# Enables the game's developer/debug startup mode.\n");
    fprintf(file, "DEBUG_MODE = %s\n\n", defaults.namcoN2.debugMode ? "true" : "false");
    fprintf(file, "# External YaCardEmu connection. YaCardEmu is never embedded in linuxloader.\n");
