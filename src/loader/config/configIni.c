...
int createDefaultIni(const char *filePath)
{
    ...
    fprintf(file, "# Whether the loader passes the force feedback the game asks for on to an\n");
    fprintf(file, "# output device. This is not the cabinet's own force feedback setting: that\n");
    fprintf(file, "# one lives in the game's test menu, and it has to stay off for now because\n");
    fprintf(file, "# the steering board's self check is not emulated yet and the cabinet\n");
    fprintf(file, "# reports PCB ERROR without it.\n");
    fprintf(file, "FFB_ENABLED = %s\n", defaults.n2ForceFeedbackEnabled ? "true" : "false");
    fprintf(file, "# Raw counts the cabinet's wheel and pedals report at each end of their\n");
    fprintf(file, "# travel. The steering initialisation screen in the test menu shows these\n");
    fprintf(file, "# divided by 64, so the defaults below read as -512..+511 for the wheel and\n");
    fprintf(file, "# 0..320 for each pedal. Widen or narrow them to match your own cabinet.\n");
    fprintf(file, "STEERING_RAW_MIN = %d\n", defaults.n2SteeringRawMin);
    fprintf(file, "STEERING_RAW_MAX = %d\n", defaults.n2SteeringRawMax);
    fprintf(file, "ACCELERATOR_RAW_MIN = %d\n", defaults.n2AcceleratorRawMin);
    fprintf(file, "ACCELERATOR_RAW_MAX = %d\n", defaults.n2AcceleratorRawMax);
    fprintf(file, "BRAKE_RAW_MIN = %d\n", defaults.n2BrakeRawMin);
    fprintf(file, "BRAKE_RAW_MAX = %d\n", defaults.n2BrakeRawMax);
    fprintf(file, "# What the loader's JVS I/O board reports about itself to the game.\n");
    fprintf(file, "# The name is free text shown in the test menu; the counts are what the\n");
    fprintf(file, "# game asks the board for, so lowering one makes it claim less.\n");
    fprintf(file, "JVS_IO_NAME = \"%s\"\n", defaults.n2JvsIoName);
    fprintf(file, "JVS_PLAYERS = %d\n", defaults.n2JvsPlayers);
    fprintf(file, "JVS_SWITCHES = %d\n", defaults.n2JvsSwitches);
    fprintf(file, "JVS_COINS = %d\n", defaults.n2JvsCoins);
    fprintf(file, "JVS_ANALOGUE_IN = %d\n", defaults.n2JvsAnalogueIn);
    fprintf(file, "JVS_GPO = %d\n", defaults.n2JvsGpo);
    fprintf(file, "JVS_ANALOGUE_OUT = %d\n", defaults.n2JvsAnalogueOut);
    fprintf(file, "JVS_GPI = %d\n", defaults.n2JvsGpi);
